...
def compute_laplacian_matrix(X, adjacency_matrix):
    """
    Computes the standard Graph Laplacian Matrix L = D - W.
    ...
    D = csr_matrix((degree_values, (range(n_vertices), range(n_vertices))), shape=(n_vertices, n_vertices))
...
def edge_collapse_decimation(X, adjacency_matrix, min_edge_length):
    """
    Performs structural decimation (E-collapse) by merging vertices 
    connected by edges shorter than min_edge_length.
    """
    n_vertices = X.shape[0]
    rows, cols = adjacency_matrix.nonzero()
    ...
    new_adj = csr_matrix((new_data, (new_rows, new_cols)), shape=(len(unique_verts), len(unique_verts)))
    
    return new_X, new_adj

def laplacian_graph_contraction(X_init, adj_init, w_L=1.0, w_H=0.1, max_iter=20, tol=1e-3, decimate_every=2, min_edge_length=0.5):
    """
    ...
    for i in range(max_iter):
        ...
        print(f"Iteration {i+1}/{max_iter} - Displacement Error: {displacement:.6f} - Nodes: {X.shape[0]}")
...
if __name__ == "__main__":
    ...
    print(f"Average Y-Z deviation from center (ideal is 0): {np.mean(np.abs(contracted_X[:, 1:])):.4f}")